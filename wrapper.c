...
static PyObject *py_CnnGate_backward(PyObject *self,PyObject * args)
{
    ...
    PyObject* p_bias;
    CnnGateParam p;
    if(!PyArg_ParseTuple(args,"O|O|O|Oii",&p_input,&p_dz,&(p_weight),&(p_bias),&(p.strids),&(p.panding)))
    {
        return NULL;
    }
...
static PyObject *py_CnnGate(PyObject *self,PyObject * args)
{
    ...
    char* p_name;
    CnnGateParam p;
    if(!PyArg_ParseTuple(args,"O|O|Oiiis",&p_object,&(p_weight),&(p_bias),&(p.strids),&(p.panding),&(p.isBackward),&p_name))
    {
        return NULL;
    }
    ParamLink* plink = NULL;
    ...
    if(p.isBackward == 1)
    {
        printShape(curp->input);
        curp->dz = pyArray2Matrix((PyArrayObject*)p_object);
        curp->_output = zeros_like(curp->dz);//!!!!!!!!!!!!!!!!!!!!!!!!!!!
    }
    else curp->input = pyArray2Matrix((PyArrayObject*)p_object);
    CnnGate(curp);
    if(p.isBackward == 1)

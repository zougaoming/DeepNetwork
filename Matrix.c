...
int PandingMatrix4D(Matrix *m,unsigned zeropanding)
{
    ...
    if(x == 0)//二维
    {
        newLength = m->length + 4*zeropanding;
    }
    else if(w == 0 && x != 0)//三维
    {
        newLength = m->length + 4*zeropanding * x;
    }
    else if(w != 0)//4维
    {
        newLength = m->length + 4*zeropanding * w;
    }
    m->dshape.shape[2] += 2*zeropanding;
    m->dshape.shape[3] += 2*zeropanding;
    m->length = newLength;
    m->size = newLength;
    m->array = (double *)realloc(m->array,(m->length)*sizeof(double));
    return 1;
}
...
int setMatrixArray(Matrix *m1,Matrix *m2,int dimen0,int dimen1){
    ...
    {
        *(m1->array + index + i) = *(m2->array + index + i);
    }
...
//求往后算ndim维度的和
double getMatrixSumbyDim(Matrix *m,int ndim,int dim0){
    if(ndim <= 0 || ndim >=4)

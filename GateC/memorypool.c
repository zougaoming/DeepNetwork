...
{
    int max_mem = 1*GB + 1000*MB + 1000*KB;
    int mem_pool_size = 800*MB + 500*KB;
    mp = MemoryPool_Init(max_mem, mem_pool_size, 1);
    return mp;
}
void 
...
void *
MemoryPool_realloc(void *hasp,mem_size_t wantsize)
{
    MemoryPool_Free(hasp);
    return MemoryPool_Alloc(wantsize);
}
